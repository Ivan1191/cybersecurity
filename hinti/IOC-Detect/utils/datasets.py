...
class CTIDatasetForBiLSTMCRF(Dataset):
    ...
    # 這個方法的目的是從輸入的句子（sentences）中建立詞彙表（vocabulary）
    # sentences: np.array：這是一個 NumPy 陣列，它包含了句子的數據。這些句子將被用來建立詞彙表。
    # 返回值 tuple[int, dict]：這個方法的返回值是一個包含整數和字典的元組。具體來說，返回的元組有兩個元素：詞彙表的大小（整數）以及詞彙表到索引的映射（字典）。
